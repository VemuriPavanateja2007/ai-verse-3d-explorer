...
export default function App() {
  ...
  const handleGenerateCustom = async (prompt: string, category?: ConceptCategory) => {
    ...
    try {
      ...
      const chosenCategory = category || (
        prompt.toLowerCase().includes('vision') || prompt.toLowerCase().includes('cnn') ? 'cnn' :
      ...
      const fallbackConcept: ConceptData = {
        ...
        architectureBreakdown: [
          {
            ...
            nodesCount: 12,
            dimensions: '12 x 1',
            description: `Receives normalized numerical features or token embeddings for ${prompt}.`
          },
          {
            id: 'l-1',
            name: `${prompt} Transformation Block`,
            role: 'Non-Linear Representation',
            nodesCount: 16,
            dimensions: '16 x 16 Matrix',
            activationFunction: 'GELU / SwiGLU',
            description: `Applies weighted matrix transformations and non-linear activations tailored for ${prompt}.`
          },
          {
            id: 'l-2',
        ...
        keyHighlights: [
          `Interactive 3D visualization custom-generated for "${prompt}"`,
