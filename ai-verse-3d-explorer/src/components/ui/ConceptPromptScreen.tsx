...
interface ConceptPromptScreenProps {
  ...
  onGenerateCustom: (prompt: string, category?: ConceptCategory) => Promise<void>;
  isLoading: boolean;
}
...
export function ConceptPromptScreen({
  ...
  const handlePresetClick = (key: ConceptCategory) => {
    if (customPrompt.trim()) {
      // If user typed a prompt and clicked a card, build that prompt in that card's architecture style!
      onGenerateCustom(customPrompt.trim(), key);
    } else {
      onSelectPreset(key);
    }
  };
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 max-w-5xl mx-auto w-full py-6">
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-3">
            Choose an Architecture or Enter a Prompt
          </h2>
          <p className="text-slate-400 text-sm sm:text-base max-w-xl mx-auto mb-6">
            Type any AI model concept below to visualize it in 3D, or choose one of our 4 default model presets.
          </p>

          {/* High-Contrast Prompt Input Tab */}
          <form onSubmit={handleCustomSubmit} className="mb-4 max-w-2xl mx-auto">
            <div className="relative flex items-center">
              ...
              <input
                ...
                placeholder="Type any custom model prompt (e.g. Diffusion Model, ResNet-50, MoE)..."
                disabled={isLoading}
                className="w-full bg-slate-900/90 text-slate-100 placeholder-slate-500 text-sm pl-12 pr-44 py-4 rounded-xl border border-indigo-500/50 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 shadow-2xl transition-all"
              />
              <button
                type="submit"
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 max-w-2xl mx-auto mb-8">
            <span className="text-[11px] text-slate-500 font-mono">Quick Prompts:</span>
            {examplePrompts.map((ex, idx) => (
              <button
                ...
                className="text-[11px] font-mono px-2.5 py-1 rounded-md bg-slate-900 hover:bg-slate-800 text-indigo-300 border border-indigo-500/30 hover:border-cyan-400/50 transition-all cursor-pointer"
              >
                + {ex}
              </button>
            ))}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700">
                        {customPrompt.trim() ? `Style: ${preset.key.toUpperCase()}` : preset.badge}
                      </span>
                    </div>
                    ...
                    <p className="text-slate-400 text-xs mb-4 leading-snug">
                      {customPrompt.trim()
                        ? `Visualize "${customPrompt.trim()}" in ${preset.title} style`
                        : preset.subtitle}
                    </p>
                  </div>
