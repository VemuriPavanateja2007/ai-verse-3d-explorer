...
export function TopNav({
  ...
  const handleNavSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (navPrompt.trim() && onGenerateCustom) {
      setIsNavLoading(true);
      await onGenerateCustom(navPrompt.trim());
  ...
  return (
    <header className="h-14 bg-slate-900/90 border-b border-slate-800/80 px-4 flex items-center justify-between z-20 backdrop-blur-md select-none">
      {/* Left Brand & Breadcrumbs & Inline Prompt Input */}
      <div className="flex items-center space-x-3">
        ...
        <div className="flex items-center space-x-2">
          <span className="font-bold text-xs sm:text-sm text-slate-100 tracking-wide truncate max-w-[160px] sm:max-w-[220px]">
            {concept.title}
          </span>
          <span className="hidden lg:inline-block text-[10px] font-mono px-2 py-0.5 rounded bg-indigo-950/80 text-indigo-300 border border-indigo-500/40">
            {concept.badge}
          </span>
        </div>

        {/* Workspace Quick Prompt Input */}
        {onGenerateCustom && (
          <form onSubmit={handleNavSubmit} className="hidden xl:flex items-center ml-2 relative">
            <Search className="w-3.5 h-3.5 text-slate-400 absolute left-2.5" />
            <input
              type="text"
              value={navPrompt}
              onChange={(e) => setNavPrompt(e.target.value)}
              placeholder="Visualize another AI prompt..."
              disabled={isNavLoading}
              className="bg-slate-950/80 text-xs text-slate-200 placeholder-slate-500 pl-8 pr-16 py-1 rounded-lg border border-slate-700 focus:border-cyan-400 focus:outline-none w-52 transition-all"
            />
            <button
              type="submit"
              disabled={isNavLoading || !navPrompt.trim()}
              className="absolute right-1 px-2 py-0.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-[10px] text-white font-medium rounded transition-all cursor-pointer flex items-center space-x-1"
            >
              {isNavLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <span>Go</span>}
            </button>
          </form>
        )}
      </div>
