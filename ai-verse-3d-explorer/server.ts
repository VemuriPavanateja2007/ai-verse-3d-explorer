    ...
    try {
      ...
      if (!ai) {
        ...
        res.json({
          ...
          architectureBreakdown: [
            {
              ...
              nodesCount: 12,
              dimensions: '12 x 1',
              description: 'Processes incoming feature embeddings or tensor inputs.',
              details: ['Normalizes continuous numerical values', 'Passes representations to feature projection layers']
            },
            {
              id: 'layer-1',
              name: 'Core Transformation Block',
              role: 'Representation Learning',
              nodesCount: 16,
              dimensions: '16 x 16 Matrix',
              activationFunction: 'GELU / Swish',
              description: 'Applies affine weight multiplications and non-linear activation transformations.',
          ...
          keyHighlights: [
            'Dynamic 3D representation tailored for custom concept query',
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Analyze the AI concept "${prompt}". Produce a clean structured JSON output explaining its architecture, category, layer breakdown, and math equations for a 3D visualization.`,
        config: {
      ...
      const validCategory = ['dnn', 'cnn', 'transformer', 'llm'].includes(parsed.category) ? parsed.category : 'dnn';
